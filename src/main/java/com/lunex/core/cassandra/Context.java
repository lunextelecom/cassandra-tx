...
public class Context implements IContext {
	...
	private static final StatsDClient statsd = new NonBlockingStatsDClient("cassandra.tx", "192.168.93.69", 8125);
		...
		if(setCfTxChanged != null && !setCfTxChanged.isEmpty()){
			for (String cf : setCfTxChanged) {
				try {
					...
					bs = prepareStatement("delete from " + Utils.getFullTXCF(cf) + " where cstx_id_ = ?",params);
					if(bs != null){
						batch.add(bs);
					}
					executeBatch( batch, false);
			...
			if(isClosed){
				params = new ArrayList<Object>();
				params.add(UUID.fromString(ctxId));
				bs = prepareStatement("delete from " + Utils.getFullTXCF("cstx_context")  + " where contextid = ?", params);
				if(bs != null){
					batch.add(bs);
				}
			}
	...
	public void executeBatch(BatchStatement batch, Boolean forceRun){
		if(forceRun){
			...
			if(batch.getStatements() != null && batch.getStatements().size()%Configuration.getBatchSize()==0){
				client.getSession().execute(batch);
				batch.clear();
			}
		}
